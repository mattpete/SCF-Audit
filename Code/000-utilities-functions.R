# Defining data path dynamically------------------------------------------------
...
tz_est <- "America/New_York"   # use Olson name; handles EST/EDT correctly
Sys.setenv(TZ = tz_est)        # base R default
options("lubridate.tz" = tz_est)


# general function to winsorize a variable in a mutate statement----------------
winsorize_x = function(x, cut = 0.01){
  cut_point_top <- quantile(x, 1 - cut, na.rm = T,type=3)
  cut_point_bottom <- quantile(x, cut, na.rm = T,type=3)
...
# general function to standardize a variable in a mutate statement-----------------
...
# function to assign Fama-French 49 industry classification --------------------
...
#Out of Sample and Performance metrics for logistic regression --------------------------------------------------------------

## splits will be the `rsplit` object with the 90/10 partition
holdout_results <- function(splits,mymodel,...) {
  # Fit the model to the 90%
  mod <- glm(mymodel, data = analysis(splits), family = "binomial")
  # Save the 10%
...
#Hold out results - this uses holdout sample to get fitted values from training AND grabs the training model number of predictions to classify positive predicitons
holdout_results_lit <- function(splits, mymodel,...) {
  #Fit the model to the training data (90%)
  mod <- glm(mymodel, data = analysis(splits), family = "binomial")
  #Save the holdout data (10%)
  holdout <- assessment(splits)
  #Get the training data
  training_data <- analysis(splits)
  #Calculate the mean of the outcome variable in the training data
  mean_outcome <- mean(training_data$class_action)
  #Predict the probabilities on the holdout data
  res <- broom::augment(mod, newdata = holdout, type.predict = "response")
  #Determine the cutoff based on the mean of the outcome variable
  cutoff <- quantile(res$.fitted, probs = 1 - mean_outcome)
  #Mark the top N percent of the fitted values as predicted positive
  res <- res |> mutate(predictions = if_else(.fitted >= cutoff, 1, 0),threshold = cutoff)
  #Return the results
  return(res)
...
#Hold out results - this uses holdout sample to get fitted values from training AND grabs the training model number of predictions to classify positive predicitons
holdout_results_settle <- function(splits, mymodel,...) {
  #Fit the model to the training data (90%)
  mod <- glm(mymodel, data = analysis(splits), family = "binomial")
  #Save the holdout data (10%)
  holdout <- assessment(splits)
  ...
  training_data <- analysis(splits)
  #Calculate the mean of the outcome variable in the training data
  mean_outcome <- mean(training_data$settled)
  #Predict the probabilities on the holdout data
  res <- broom::augment(mod, newdata = holdout, type.predict = "response")
  #Determine the cutoff based on the mean of the outcome variable
  cutoff <- quantile(res$.fitted, probs = 1 - mean_outcome)
  #Mark the top N percent of the fitted values as predicted positive
  res <- res |> mutate(predictions = if_else(.fitted >= cutoff, 1, 0))
  #Return the results
  return(res)
}


#Change the variable "SCA/Litigation" to match predicted Y variable
logit_performance<-function(results,threshold,DV,input_data){

  cutoff <- case_when(threshold == "Number" ~ min(results |> arrange(desc(.fitted)) |>  head(sum(input_data[[DV]] == 1)) |>  pull(.fitted)),
                      threshold == "Mean" ~ mean(input_data[[DV]]),
                      TRUE ~ 0.5)
  
  d1 <-  results |> 
    mutate(predictions = if_else(.fitted > cutoff, 1, 0),
           # Calculate whether the prediction was correct
           TP = if_else(predictions == 1 & get(DV) == 1,1,0),
           FP = if_else(predictions == 1 & get(DV) == 0,1,0),
           TN = if_else(predictions == 0 & get(DV) == 0,1,0),
           FN = if_else(predictions == 0 & get(DV) == 1,1,0)) |> 
    summarise(
      TP = sum(TP),
      FP = sum(FP),
      TN = sum(TN),
      FN = sum(FN)) |> 
    summarise(Accuracy = (TP+TN)/(TP+FP+TN+FN),
              Precision = TP/(TP + FP),
              Recall = TP/(TP + FN),
  ...
  d2 <- results |> 
    mutate(truth = factor(input_data[[DV]], levels = c(0, 1))) |> 
    roc_auc(truth = truth, .fitted, event_level = "second") |> 
    summarise(AUC = .estimate)
...
logit_performance_os<-function(results,DV){
  ...
  #d2 <- tibble(AUC = roc_auc(results,!!sym(DV),.fitted,event_level = "second")$.estimate)
  d2 <- results |> 
    mutate(truth = factor(get(DV), levels = c(0, 1))) |> 
    roc_auc(truth = truth, .fitted, event_level = "second") |> 
    summarise(AUC = .estimate)
...
##Set options and LATEX formatting functions-------------------------------------------------

options(modelsummary_format_numeric_latex = "plain")

#set number formats
my_fmt <- function(x) formattable::comma(x, digits=2)
...
gm_felm <- list(
  list("raw" = "nobs", "clean" = "Num. Obs.", "fmt" = nobs_fmt),
  list("raw" = "adj.r.squared", "clean" = "Adj. $R^2$", "fmt" = 3),
  list("raw" = "r2.within.adjusted", "clean" = "Adj. $R^2$ Within", "fmt" = 3))
...
message("Successfully set R formatting options and loaded functions")



#Summary function with more quantiles-------------


summary_q <- function(x, probs = c(0.05, 0.10, 0.25, 0.50, 0.75, 0.90, 0.95)) {
  ...
  if (is.data.frame(x)) {
    ...
    lapply(x, summary_q, probs = probs)
  } else {
    qs <- quantile(x, probs = probs, na.rm = TRUE)
    lower <- qs[probs < 0.5]
    upper <- qs[probs > 0.5]
    
    c(N = round(sum(!is.na(x)),0),
      Min = min(x, na.rm = TRUE),
      setNames(lower, paste0("P", probs[probs < 0.5] * 100)),
      Mean = mean(x, na.rm = TRUE),
      Median = median(x, na.rm = TRUE),
      setNames(upper, paste0("P", probs[probs > 0.5] * 100)),
